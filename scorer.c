...
int main(int argc, char* argv[]) {
	...
	Score scorer[MAX_REPORTS_COUNT] = {0};
	...
	while((bytes_read = read(fd, &r, sizeof(Report))) == sizeof(Report)) {
		...
		if(new_inspector) {
			if(pointer_scorer >= MAX_REPORTS_COUNT) {
				fprintf(stderr, "ERROR: too many inspectors!\n");
				close(fd);
				exit(EXIT_FAILURE);
			}

			pointer_for_inspector = pointer_scorer;
			pointer_scorer++;
			...
			scorer[pointer_for_inspector].inspector[inspector_size - 1] = '\0';
		}
